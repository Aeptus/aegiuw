...
/**
 * aegiuw-router — the stateless edge traffic controller (PRD §1.2).
 *
 * Responsibilities (most still TODO at this scaffold stage):
 *  - ingest isolate requests from `aegiuw-daemon` (Condition B, §1.1),
...
export interface Env {
  /** Browser Rendering binding — the ephemeral Chromium sandbox (aegiuw-cage, §1.3).
...
interface IsolateRequest {
  ...
  url: string;
  /** The risk level the local engine assigned (mirrors aegiuw-core `RiskLevel`). */
...
async function handleIsolate(request: Request, _env: Env): Promise<Response> {
  ...
  // TODO(FR-3): use env.AEGIUW_CAGE to launch a blank-profile headless Chromium,
...
export default {
  async fetch(request: Request, env: Env, _ctx: ExecutionContext): Promise<Response> {
    ...
    switch (route) {
      case "GET /health":
        return json({ status: "ok", service: "aegiuw-router", version: "0.1.0" });
      case "POST /isolate":
