...
- **P1 (P2) `SniOutcome::Cleartext { host: Cow<'a, str> }`.** Deferred with rationale. Returning `Cow<'a, str>` would let callers who don't need to outlive the input buffer avoid the one host-String allocation per `Cleartext`. The savings: ~30–50 bytes per ClientHello. The cost: a lifetime parameter on `SniOutcome` cascading through `extract_sni<'a>(bytes: &'a [u8]) -> SniOutcome<'a>`, `hrr_sni_consistent`, every test fixture, every caller, and the serde derive (Cow has different round-trip semantics than String). **Conclusion:** premature optimisation for our daemon use case where we parse ~10–30 ClientHellos per user per second on the isolate path. Re-evaluate if profile data (P4) shows the allocation as a real hot spot, or if the Worker (which has stricter perf budgets) demands it. The lifetime would also clash with O1's tracing event, which currently borrows `bytes.len()` and would have to either copy outcome bytes or refactor again.
